...
class BinauralBeatGenerator {
    ...
    func generateBeat(frequency: Double, duration: TimeInterval) -> AVAudioPCMBuffer? {
        ...
        guard let format = AVAudioFormat(standardFormatWithSampleRate: sampleRate, channels: 2) else {
            print("Failed to create AVAudioFormat with sampleRate: \(sampleRate)")
            return nil
        }
    ...
    private func createEmptyBuffer() -> AVAudioPCMBuffer {
        ...
        return AudioBufferPool.shared.obtainBuffer(format: format, frameCapacity: 1024)
    }
}
...
class NatureSoundGenerator {
    private let sampleRate: Double
    private let audioEngine = AVAudioEngine() // Audio engine for sample playback and effects
    private var playerNodes: [AVAudioPlayerNode] = [] // Array to hold audio player nodes
    private var environmentSamplers: [NatureEnvironment: Sampler] = [:] // Samplers for different environments
    
    init(sampleRate: Double) {
        self.sampleRate = sampleRate
        
        // Initialize samplers for each environment
        for environment in NatureEnvironment.allCases {
            environmentSamplers[environment] = Sampler(for: environment, sampleRate: sampleRate)
        }
    }
    
    func generateNatureSound(environment: NatureEnvironment, duration: TimeInterval, dynamicVariations: Bool = true) -> AVAudioPCMBuffer? {
        let frameCount = AVAudioFrameCount(sampleRate * duration)
        guard let format = AVAudioFormat(standardFormatWithSampleRate: sampleRate, channels: 2) else {
            print("Failed to create AVAudioFormat with sampleRate: \(sampleRate)")
            return nil
        ...
        switch environment {
        case .ocean:
            generateOceanSounds(leftChannel: leftChannel, rightChannel: rightChannel, frameCount: Int(frameCount), dynamicVariations: dynamicVariations)
        case .rain:
            generateRainSounds(leftChannel: leftChannel, rightChannel: rightChannel, frameCount: Int(frameCount), dynamicVariations: dynamicVariations)
        case .forest:
            generateForestSounds(leftChannel: leftChannel, rightChannel: rightChannel, frameCount: Int(frameCount), dynamicVariations: dynamicVariations)
        case .fire:
            generateFireSounds(leftChannel: leftChannel, rightChannel: rightChannel, frameCount: Int(frameCount), dynamicVariations: dynamicVariations)
        }
        
        return buffer
    }
    
    private func generateOceanSounds(leftChannel: UnsafeMutablePointer<Float>, rightChannel: UnsafeMutablePointer<Float>, frameCount: Int, dynamicVariations: Bool) {
        // Use granular synthesis for ocean waves
        let grainDuration: TimeInterval = 0.05 // Duration of each grain
        let grainsPerSecond = 10 // Number of grains per second
        let overlap = 2 // Overlap factor for grains
        
        let grainFrameCount = Int(grainDuration * sampleRate)
        let grainSpacing = Int(sampleRate / Double(grainsPerSecond))
        
        // Pre-calculate wavetables for granular synthesis
        let wavetableSize = 2048
        let wavetable1 = generateWavetable(size: wavetableSize, type: .sine)
        let wavetable2 = generateWavetable(size: wavetableSize, type: .triangle)
        
        for i in stride(from: 0, to: frameCount, by: grainSpacing / overlap) {
            let time = Double(i) / sampleRate
            let envelope = calculateNatureEnvelope(frame: i, totalFrames: Int(frameCount), time: time)
            
            // Dynamically vary wave frequencies and amplitudes
            let waveFreq1 = dynamicVariations ? 0.3 + 0.2 * sin(time * 0.1) : 0.3
            let waveFreq2 = dynamicVariations ? 8.0 + 4.0 * sin(time * 0.3) : 8.0
            let waveAmp1 = dynamicVariations ? 0.4 + 0.1 * cos(time * 0.05) : 0.4
            let waveAmp2 = dynamicVariations ? 0.3 + 0.1 * sin(time * 0.08) : 0.3
            
            // Generate grains using wavetables
            let grain1 = generateGrain(wavetable: wavetable1, frequency: waveFreq1, grainFrameCount: grainFrameCount)
            let grain2 = generateGrain(wavetable: wavetable2, frequency: waveFreq2, grainFrameCount: grainFrameCount)
            
            // Mix grains and apply envelope
            for j in 0..<grainFrameCount {
                if i + j < frameCount {
                    let grainSample = (grain1[j] * Float(waveAmp1) + grain2[j] * Float(waveAmp2)) * Float(envelope)
                    leftChannel[i + j] += grainSample * 0.3
                    rightChannel[i + j] += grainSample * 0.3
                }
            }
        }
    }
