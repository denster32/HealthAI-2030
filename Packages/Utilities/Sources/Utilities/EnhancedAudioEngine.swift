...
class EnhancedAudioEngine: NSObject, ObservableObject {
    ...
    private func setupAdvancedNodes() {
        ...
        if let compressorNode = AVAudioUnitEffect(audioComponentDescription: compressorDesc) {
            ...
            if let dynamicsProcessor = compressorNode as? AVAudioUnitDynamicsProcessor {
                dynamicsProcessor.threshold = -20.0 // dB
                dynamicsProcessor.ratio = 4.0 // Ratio of compression
                dynamicsProcessor.attackTime = 0.01 // Seconds
                dynamicsProcessor.releaseTime = 0.05 // Seconds
                dynamicsProcessor.knee = 5.0 // dB
            }
            
            // Configure limiter parameters
            if let peakLimiter = limiterNode as? AVAudioUnitPeakLimiter {
                peakLimiter.ceiling = -1.0 // dB
                peakLimiter.threshold = -10.0 // dB
                peakLimiter.attackTime = 0.005 // Seconds
                peakLimiter.releaseTime = 0.025 // Seconds
            }
        }
