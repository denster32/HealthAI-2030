...
class MetalChartRenderer: NSObject, ObservableObject {
    ...
    private let isMPS3Available: Bool
    ...
    private override init() {
        ...
        self.isMPS3Available = {
            if #available(iOS 18, macOS 15, *) {
                return true // MPS 3.0 available
            } else {
                return false
            }
        }()
        ...
        Logger.info("MetalChartRenderer initialized with Metal support: \(metalDevice != nil), MPS 3.0: \(isMPS3Available)", log: Logger.performance)
    }
    ...
    private func setupMetalRenderPipelines() {
        ...
        do {
            if isMPS3Available {
                // TODO: Use new MPS 3.0 APIs for even faster rendering (iOS 18+/macOS 15+)
                Logger.info("MPS 3.0 optimizations enabled for chart rendering", log: Logger.performance)
                // Example: Use new blending, primitives, or compute shaders here
            }
