...
// Supporting types
struct TrendAnalysis { let values: [Double]; let trend: TrendDirection }
struct CorrelationInsight { let metricA: String; let metricB: String; let correlation: Double }
struct EnvironmentSnapshot { let temperature: Double; let airQuality: Double; let noiseLevel: Double; let timestamp: Date }
struct HistoricalPrediction { let predicted: Double; let actual: Double; let timestamp: Date }
struct ActualOutcome { let value: Double; let timestamp: Date }
struct WeekdayWeekendPattern { let weekdayAvg: Double; let weekendAvg: Double; let difference: Double }
struct SedentaryPeriod { let start: Date }
struct StressPeriod { let start: Date }
struct StressRecoveryPattern { let avgRecoveryTime: Double; let recoveryEffectiveness: Double }
struct AbnormalReading { let timestamp: Date; let value: Double }
...
struct AnalyticsReport { let summary: String; let details: [String: Any] }
