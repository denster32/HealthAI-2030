    ...
    func saveSleepData(_ sleepData: SleepData) async throws {
        ...
        try await operationQueue.async {
            ...
            backgroundContext.performAndWait {
                ...
                entity.startDate = sleepData.startTime
                entity.endDate = sleepData.endTime
                entity.timestamp = Date()
                entity.sleepStage = Int16(sleepData.stages.first?.rawValue ?? 4) // 4 = unknown
            ...
            backgroundContext.performAndWait {
                ...
                do {
                    ...
                    sleepData = entities.compactMap { entity -> SleepData? in
                        guard let startTime = entity.startDate,
                              let endTime = entity.endDate else { return nil }
                        
                        let duration = endTime.timeIntervalSince(startTime)
                        let stage = SleepStage(rawValue: Int(entity.sleepStage)) ?? .unknown
                        ...
                        return SleepData(
                            ...
                            endTime: endTime,
                            duration: duration,
                            quality: 0.0, // Default quality since it's not stored in the model
                            stages: [stage]
                        )
                    }
                } catch {
            ...
            backgroundContext.performAndWait {
                ...
                for i in stride(from: 0, to: healthData.count, by: batchSize) {
                    ...
                    for dataPoint in batch {
                        ...
                        // Map the data point type to the appropriate attribute
                        switch dataPoint.type {
                        case .heartRate:
                            entity.heartRate = dataPoint.value
                        case .hrv:
                            entity.hrv = dataPoint.value
                        case .oxygenSaturation:
                            entity.oxygenSaturation = dataPoint.value
                        case .bodyTemperature:
                            entity.bodyTemperature = dataPoint.value
                        case .stepCount:
                            entity.stepCount = Int32(dataPoint.value)
                        case .activeEnergyBurned:
                            entity.activeEnergyBurned = dataPoint.value
                        default:
                            // For unknown types, store in heartRate as fallback
                            entity.heartRate = dataPoint.value
                        }
                    }
        ...
        context.performAndWait {
            do {
                ...
                if let sleepEntity = sleepEntity,
                   let startTimeProperty = sleepEntity.propertiesByName["startTime"],
                   let endTimeProperty = sleepEntity.propertiesByName["endTime"] {
                    let startTimeIndex = NSFetchIndexElement(property: startTimeProperty, collationType: .binary)
                    let endTimeIndex = NSFetchIndexElement(property: endTimeProperty, collationType: .binary)
                ...
                if let healthEntity = healthEntity,
                   let timestampProperty = healthEntity.propertiesByName["timestamp"] {
                    let timestampIndex = NSFetchIndexElement(property: timestampProperty, collationType: .binary)
...
// Note: SleepDataEntity and HealthDataEntity are automatically generated by Core Data
// from the HealthAI2030.xcdatamodeld file
