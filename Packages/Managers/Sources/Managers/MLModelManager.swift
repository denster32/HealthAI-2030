...
class MLModelManager: ObservableObject {
    ...
    private func createSleepStageInput(features: SleepStageFeatures) -> MLFeatureProvider {
        ...
        do {
            ...
            print("Error creating sleep stage feature provider: \(error)")
            // Return an empty feature provider as fallback
    ...
    private func createArrhythmiaInput(ecgData: [Double]) -> MLFeatureProvider {
        ...
        do {
            let multiArray = try MLMultiArray(shape: [1, NSNumber(value: ecgData.count)], dataType: .double)
            ...
            let featureDict: [String: MLFeatureValue] = [
                "ecgData": MLFeatureValue(multiArray: multiArray)
            ]
            
            return try MLDictionaryFeatureProvider(dictionary: featureDict)
        } catch {
            print("Error creating arrhythmia input: \(error)")
            // Return an empty feature provider as fallback
            return try! MLDictionaryFeatureProvider(dictionary: [:])
        }
    }
    ...
    private func createHealthPredictionInput(features: HealthPredictionFeatures) -> MLFeatureProvider {
        ...
        do {
            ...
            print("Error creating health prediction input: \(error)")
            // Return an empty feature provider as fallback
            return try! MLDictionaryFeatureProvider(dictionary: [:])
        }
    }
    ...
    private func createDigitalTwinInput(scenario: HealthScenario) -> MLFeatureProvider {
        ...
        do {
            ...
            print("Error creating digital twin input: \(error)")
            // Return an empty feature provider as fallback
            return try! MLDictionaryFeatureProvider(dictionary: [:])
        }
    }
...
// Using SleepStageType from HealthModels.swift instead of duplicating
import Models

// This typealias helps with backward compatibility
typealias SleepStage = SleepStageType
