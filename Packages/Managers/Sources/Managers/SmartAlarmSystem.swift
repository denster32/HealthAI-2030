...
class SmartAlarmSystem: NSObject, ObservableObject {
    ...
    func calculateWakeupReadiness() async -> Float {
        ...
        readiness += durationScore * (wakeupReadinessFactors["sleepDuration"] ?? 0.2)
        
        // Sleep quality factor
        let qualityScore = sleepMetrics.quality
        readiness += qualityScore * (wakeupReadinessFactors["sleepQuality"] ?? 0.25)
        
        // Sleep cycles factor
        let cyclesScore = calculateCyclesScore(sleepMetrics.cycles)
        readiness += cyclesScore * (wakeupReadinessFactors["sleepCycles"] ?? 0.2)
        
        // Biometric trends factor
        let biometricScore = calculateBiometricScore(biometricTrends)
        readiness += biometricScore * (wakeupReadinessFactors["biometricTrends"] ?? 0.2)
        
        // Sleep debt factor
        let debtScore = calculateDebtScore(sleepDebt)
        readiness += debtScore * (wakeupReadinessFactors["sleepDebt"] ?? 0.15)
...
class SleepCycleAnalyzer {
    ...
    private func predictCycleQuality(for cycleIndex: Int, patterns: [SleepPattern]) -> Double {
        ...
        guard let recentPattern = patterns.last ?? patterns.first else {
            return 0.7
        }
