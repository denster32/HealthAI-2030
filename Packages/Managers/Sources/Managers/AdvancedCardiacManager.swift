...
class AdvancedCardiacManager: ObservableObject {
    ...
    private func fetchAFibBurden() {
        guard let afibBurdenType = HKQuantityType.quantityType(forIdentifier: .atrialFibrillationBurden) else {
            print("AFib Burden type not available")
            return
        }
        ...
        let query = HKSampleQuery(sampleType: afibBurdenType, predicate: predicate, limit: 1, sortDescriptors: [sortDescriptor]) { [weak self] query, samples, error in
            DispatchQueue.main.async {
                if let error = error {
                    print("Error fetching AFib burden: \(error.localizedDescription)")
                    return
                }
    ...
    private func fetchCardioFitness() {
        guard let cardioFitnessType = HKQuantityType.quantityType(forIdentifier: .cardioFitness) else {
            print("Cardio Fitness type not available")
            return
        }
        ...
        let query = HKSampleQuery(sampleType: cardioFitnessType, predicate: predicate, limit: 1, sortDescriptors: [sortDescriptor]) { [weak self] query, samples, error in
            DispatchQueue.main.async {
                if let error = error {
                    print("Error fetching Cardio Fitness: \(error.localizedDescription)")
                    return
                }
                if let sample = samples?.first as? HKQuantitySample {
    ...
    private func fetchVO2Max() {
        guard let vo2MaxType = HKQuantityType.quantityType(forIdentifier: .vo2Max) else {
            print("VO2 Max type not available")
            return
        }
    ...
    private func fetchIrregularRhythmEvents() {
        guard let irregularRhythmType = HKObjectType.categoryType(forIdentifier: .irregularHeartRhythm) else {
            print("Irregular Rhythm type not available")
            return
        }
        let predicate = HKQuery.predicateForSamples(withStart: Date().addingTimeInterval(-7 * 24 * 3600), end: Date(), options: .strictEndDate)
        let sortDescriptor = NSSortDescriptor(key: HKSampleSortIdentifierStartDate, ascending: false)
        let query = HKSampleQuery(sampleType: irregularRhythmType, predicate: predicate, limit: HKObjectQueryNoLimit, sortDescriptors: [sortDescriptor]) { [weak self] query, samples, error in
            DispatchQueue.main.async {
                if let error = error {
                    print("Error fetching Irregular Rhythm Events: \(error.localizedDescription)")
                    return
                }
...
class CardiacAnalyzer {
    ...
    func calculateCardiacRiskScore(afibBurden: Double, vo2Max: Double, heartRateData: [HeartRateSample], hrvData: [HRVSample], afibEpisodes: [AFibEpisode]) async -> Double {
        // Weighted risk calculation based on clinical guidelines
        ...
        // Weight: AFib 30%, Fitness 20%, HR 25%, HRV 25%
        return afibRisk * 0.3 + fitnessRisk * 0.2 + heartRateRisk * 0.25 + hrvRisk * 0.25
    }
    func analyzeCardiacHealthTrend(heartRateData: [HeartRateSample], hrvData: [HRVSample], afibEpisodes: [AFibEpisode], cardioFitnessHistory: [CardioFitnessRecord]) async -> CardiacHealthTrend {
        // Analyze trends: improving, declining, or stable
        let hrTrend = trendDirection(heartRateData.map { $0.value })
        let hrvTrend = trendDirection(hrvData.map { $0.value })
        let afibTrend = afibEpisodes.count > 1 ? .declining : .stable
        if hrTrend == .declining && hrvTrend == .improving && afibTrend == .stable {
            return .improving
        } else if hrTrend == .improving && hrvTrend == .declining {
            return .declining
        }
        return .stable
    }
    func generateInsights(afibBurden: Double, vo2Max: Double, heartRateData: [HeartRateSample], hrvData: [HRVSample], afibEpisodes: [AFibEpisode]) async -> [CardiacInsight] {
        ...
        if afibBurden > 5.0 {
            insights.append(CardiacInsight(message: "High AFib burden detected. Consult your cardiologist."))
        }
        if vo2Max < 30 {
            insights.append(CardiacInsight(message: "Low VO2 Max. Consider increasing aerobic activity."))
        }
        if let avgHR = heartRateData.map({ $0.value }).average(), avgHR > 100 {
            insights.append(CardiacInsight(message: "Elevated average heart rate. Monitor for tachycardia."))
        }
        if let avgHRV = hrvData.map({ $0.value }).average(), avgHRV < 20 {
            insights.append(CardiacInsight(message: "Low HRV detected. Focus on stress reduction and recovery."))
        }
        return insights
    }
    ...
    private func calculateHRVRisk(_ hrvData: [HRVSample]) -> Double {
        ...
        let averageHRV = hrvData.map { $0.value }.average() ?? 40
        return averageHRV < 20 ? 0.8 : 0.2
    }
    private func trendDirection(_ values: [Double]) -> CardiacHealthTrend {
        guard values.count > 1 else { return .stable }
        let diff = values.last! - values.first!
...
class AFibDetector {
    ...
    func detectPatterns(afibBurden: Double, afibEpisodes: [AFibEpisode], heartRateData: [HeartRateSample]) async -> [AFibPattern] {
        // Detect AFib patterns using episode clustering and HR variability
        var patterns: [AFibPattern] = []
        let frequentEpisodes = afibEpisodes.filter { $0.duration > 5 * 60 }
        if !frequentEpisodes.isEmpty {
            patterns.append(AFibPattern(description: "Frequent long AFib episodes detected."))
        }
        let highHRV = heartRateData.filter { $0.value > 120 }
        if !highHRV.isEmpty {
            patterns.append(AFibPattern(description: "Episodes with high heart rate variability detected."))
        }
        return patterns
    }
}
class FitnessAnalyzer {
    ...
    func generateRecommendations(vo2Max: Double, fitnessAge: Int, cardioFitness: Double) async -> [FitnessRecommendation] {
        ...
        if vo2Max < 35 {
            recs.append(FitnessRecommendation(message: "Increase aerobic exercise to improve VO2 Max."))
        }
        if fitnessAge > 50 {
            recs.append(FitnessRecommendation(message: "Incorporate strength training to reduce fitness age."))
        }
        if cardioFitness < 0.5 {
            recs.append(FitnessRecommendation(message: "Increase weekly cardio sessions for better fitness."))
        }
        return recs
    }
}