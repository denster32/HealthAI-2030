...
struct DashboardView: View {
    ...
    // Updated to use @Observable for iOS 17+
    @State private var healthDataManager = HealthDataManager.shared
    @State private var predictiveAnalytics = PredictiveAnalyticsManager.shared
    @State private var sleepOptimizationManager = SleepOptimizationManager.shared
    @State private var environmentManager = EnvironmentManager.shared
    @State private var realTimeSyncManager = RealTimeSyncManager.shared
    @State private var appleWatchManager = AppleWatchManager.shared
    ...
    // Improved Query with better filtering
    @Query(sort: [SortDescriptor(\HealthData.timestamp, order: .reverse)], animation: .default) 
    private var healthData: [HealthData]
    
    @AppStorage("selectedTimeRange") private var selectedTimeRange: TimeRange = .day
    @State private var showingHealthDetails = false
    @State private var showingHealthAlerts = false
    @State private var showingSyncStatus = false
    
    // New property for animation state
    ...
    private func refreshData() async {
        ...
        do {
            ...
            async let healthRefresh = healthDataManager.refreshHealthData()
            async let analyticsRefresh = predictiveAnalytics.refreshAnalytics()
            async let sleepRefresh = sleepOptimizationManager.refreshSleepData()
            async let environmentRefresh = environmentManager.refreshEnvironmentData()
            
            // Wait for all refreshes to complete
            _ = try await (healthRefresh, analyticsRefresh, sleepRefresh, environmentRefresh)
...
struct HealthMetricCard: View {
        ...
        VStack(spacing: 12) {
            HStack {
                Image(systemName: icon)
                    ...
                    .symbolEffect(.pulse, options: .repeat(1), value: animationTrigger)
    ...
    private var trendIcon: some View {
        Image(systemName: trend.iconName)
            ...
            .symbolEffect(.bounce, options: .repeat(1), value: animationTrigger)
    }
}
