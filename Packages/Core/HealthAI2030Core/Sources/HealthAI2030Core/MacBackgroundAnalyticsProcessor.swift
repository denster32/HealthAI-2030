...
class MacBackgroundAnalyticsProcessor: ObservableObject {
    ...
    @Published var systemResourceUsage: BackgroundAnalyticsResourceUsage = BackgroundAnalyticsResourceUsage()
    ...
    private func completeJob(_ job: AnalyticsJob, result: JobResult) {
        ...
        // Sync results to iCloud for cross-device availability
        Task {
            do {
                try await cloudKitSync.syncJobResult(completedJob)
                os_log("Successfully synced job result to iCloud", log: .default, type: .info)
            } catch {
                os_log("Failed to sync job result to iCloud: %@", log: .default, type: .error, error.localizedDescription)
            }
        }
    ...
    private func updateSystemResourceUsage() {
        ...
        systemResourceUsage = BackgroundAnalyticsResourceUsage(
    ...
    private func prepareTrainingData(for job: AnalyticsJob) throws -> TrainingDataSet {
        ...
        // Get health data from the specified time range
        let startDate = Date().addingTimeInterval(-timeRange)
        let endDate = Date()
        
        do {
            let healthData = try healthDataManager.fetchHealthData(from: startDate, to: endDate)
            return TrainingDataSet(healthData: healthData)
        } catch {
            os_log("Failed to fetch health data for training: %@", log: .default, type: .error, error.localizedDescription)
            throw AnalyticsProcessingError.dataPreparationFailed(error)
        }
    }
...
struct CompletedJob: Identifiable {
    ...
    let endTime: Date
    let result: JobResult
    let resourcesUsed: BackgroundAnalyticsResourceUsage
...
struct BackgroundAnalyticsResourceUsage {
    let cpuUsage: Double
