...
class SiriRemoteGestureManager: NSObject, ObservableObject {
    ...
    @objc private func handlePanGesture(_ gesture: UIPanGestureRecognizer) {
        ...
        case .changed:
            ...
            let velocityMagnitudeSquared = velocity.x * velocity.x + velocity.y * velocity.y
            let velocityMagnitude = sqrt(velocityMagnitudeSquared)
            let normalizedVelocity = Float(velocityMagnitude) / 1000.0
            gestureIntensity = min(normalizedVelocity, 1.0)
...
struct HealthDataLayer {
    ...
    let icon: String
    let color: Color
    let dataType: SiriHealthDataType
    let visualizationType: VisualizationType
}

enum SiriHealthDataType {
    case heartRate
    case hrv
    case sleep
    case activity
...
struct HealthDataLayerView: View {
    ...
    var body: some View {
        let iconView = Image(systemName: layer.icon)
            .font(.title)
