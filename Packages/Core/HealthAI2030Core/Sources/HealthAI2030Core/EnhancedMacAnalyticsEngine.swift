...
public class EnhancedMacAnalyticsEngine: ObservableObject {
    ...
    @Published public var systemResources: EnhancedAnalyticsResourceUsage = EnhancedAnalyticsResourceUsage()
    ...
    private init() {
        ...
        startBackgroundAnalyticsTask()
        startResourceMonitoringTask()
    ...
    // MARK: - Modernized Background Analytics Scheduling
    private var backgroundAnalyticsTask: Task<Void, Never>?
    private func startBackgroundAnalyticsTask() {
        backgroundAnalyticsTask?.cancel()
        backgroundAnalyticsTask = Task.detached(priority: .background) { [weak self] in
            while true {
                let now = Date()
                let calendar = Calendar.current
                let next2AM = calendar.nextDate(after: now, matching: DateComponents(hour: 2, minute: 0), matchingPolicy: .nextTimePreservingSmallerComponents) ?? now.addingTimeInterval(24*60*60)
                let sleepInterval = next2AM.timeIntervalSince(now)
                try? await Task.sleep(nanoseconds: UInt64(sleepInterval * 1_000_000_000))
                await MainActor.run {
                    self?.logger.info("Starting scheduled background analytics at 2 AM")
                    Task { await self?.performScheduledAnalytics() }
                }
            }
        }
    }
    ...
    // MARK: - Modernized Resource Monitoring
    private var resourceMonitoringTask: Task<Void, Never>?
    private func startResourceMonitoringTask() {
        resourceMonitoringTask?.cancel()
        resourceMonitoringTask = Task.detached(priority: .background) { [weak self] in
            while true {
    ...
    private func updateSystemResources() {
        ...
        systemResources = EnhancedAnalyticsResourceUsage(
        ...
        logger.info("System resources updated: CPU \(systemResources.cpuUsage), Memory \(systemResources.memoryUsage)")
    }
...
public class MLModelManager {
    ...
    // Store loaded models
    public private(set) var healthPredictorModel: MLModel?
    public private(set) var sleepStageClassifierModel: MLModel?
    public private(set) var anomalyDetectionModel: MLModel?
    
    func enableAppleSiliconOptimizations() {
        logger.info("MLModelManager: Apple Silicon optimizations enabled.")
    }

    /// Loads all required CoreML models from the app bundle. Falls back to rule-based analytics if loading fails.
    func loadHealthAnalyticsModels() {
        healthPredictorModel = loadModel(named: "HealthPredictor")
        sleepStageClassifierModel = loadModel(named: "SleepStageClassifier")
        anomalyDetectionModel = loadModel(named: "AnomalyDetector")
        
        if healthPredictorModel == nil || sleepStageClassifierModel == nil {
            logger.error("MLModelManager: One or more critical models failed to load. Falling back to rule-based analytics.")
            // Optionally set up rule-based fallback here
        } else {
            logger.info("MLModelManager: Health analytics models loaded successfully.")
        }
    }

    /// Loads a specific CoreML model from the app bundle.
    private func loadModel(named modelName: String) -> MLModel? {
        guard let modelURL = Bundle.main.url(forResource: modelName, withExtension: "mlmodelc") else {
            logger.error("MLModelManager: Model \(modelName) not found in bundle.")
            return nil
        }
        do {
            let model = try MLModel(contentsOf: modelURL)
            logger.info("MLModelManager: Loaded model \(modelName).")
            return model
        } catch {
            logger.error("MLModelManager: Failed to load model \(modelName): \(error.localizedDescription)")
            return nil
        }
    }

    /// Loads the anomaly detection model asynchronously, with fallback.
    func loadAnomalyDetectionModel() async throws -> MLModel? {
        if let model = anomalyDetectionModel {
            return model
        }
        guard let modelURL = Bundle.main.url(forResource: "AnomalyDetector", withExtension: "mlmodelc") else {
            logger.error("MLModelManager: AnomalyDetector model not found in bundle.")
            throw AnalyticsError.modelLoadingFailed
        }
        do {
            let model = try MLModel(contentsOf: modelURL)
            logger.info("MLModelManager: Anomaly detection model loaded successfully.")
            return model
        } catch {
...
public struct EnhancedAnalyticsResourceUsage {
