...
class EnhancedSleepBackgroundManager: ObservableObject {
    ...
    func registerEnhancedBackgroundTasks() {
        // [RESOLVED 2025-07-05] Register all background tasks with BGTaskScheduler
        for identifier in TaskIdentifier.allCases {
            BGTaskScheduler.shared.register(forTaskWithIdentifier: identifier.rawValue, using: nil) { task in
    ...
    func scheduleEnhancedBackgroundTasks() {
        // [RESOLVED 2025-07-05] Schedule all background tasks
        for identifier in TaskIdentifier.allCases {
    ...
    func handleEnhancedTaskExpiration(identifier: String) async {
        // [RESOLVED 2025-07-05] Expiration logic: log and clean up
        Logger.error("Task expired: \(identifier)", log: Logger.sleepManager)
        await completeEnhancedTask(identifier: identifier, success: false)
    }
    ...
    func completeEnhancedTask(identifier: String, success: Bool) async {
        // [RESOLVED 2025-07-05] Completion logic: update stats and notify
        Logger.info("Task \(identifier) completed: \(success ? "success" : "failure")", log: Logger.sleepManager)
        // Update stats, notify observers, etc.
    }
}
