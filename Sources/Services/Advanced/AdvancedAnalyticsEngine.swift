...
class DataProcessor {
    ...
    /// Normalizes features using single-pass matrix optimization
    private func normalizeFeatures(_ features: [[Double]]) -> [[Double]] {
        guard !features.isEmpty, let firstRow = features.first, !firstRow.isEmpty else { 
            return features 
        }
        
        let rowCount = features.count
        let colCount = firstRow.count
        
        // Pre-calculate min/max for each column in a single pass
        var columnStats: [(min: Double, max: Double)] = Array(repeating: (Double.greatestFiniteMagnitude, -Double.greatestFiniteMagnitude), count: colCount)
        
        // Single pass to find min/max for each column
        for row in features {
            for (colIndex, value) in row.enumerated() {
                columnStats[colIndex].min = min(columnStats[colIndex].min, value)
                columnStats[colIndex].max = max(columnStats[colIndex].max, value)
            }
        }
        
        // Normalize in a single pass with pre-allocated array
        var normalizedFeatures = Array(repeating: Array(repeating: 0.0, count: colCount), count: rowCount)
        
        for (rowIndex, row) in features.enumerated() {
            for (colIndex, value) in row.enumerated() {
                let (minVal, maxVal) = columnStats[colIndex]
                normalizedFeatures[rowIndex][colIndex] = (maxVal != minVal) ? (value - minVal) / (maxVal - minVal) : 0.0
            }
        }
