...
extension SecureExportStorage {
    ...
    /// GDPR compliance: Data portability export
    public func exportUserDataForGDPR(userId: String) async throws -> GDPRExportResult {
        logger.info("Starting GDPR data export for user: \(userId)")
        
        // Validate user consent for data export
        guard await validateUserConsent(userId: userId, purpose: "data-export") else {
            throw SecureExportError.consentRequired
        }
        
        // Collect all user data
        let userData = try await collectUserData(userId: userId)
        
        // Encrypt the export data
        let encryptedData = try encryptExportData(userData)
        ...
        let export = GDPRExport(
            ...
            dataCategories: userData.keys.map { $0 }
        )
        
        // Store the export securely
        try await storeExport(export)
        ...
        return GDPRExportResult(
            ...
            downloadUrl: generateDownloadURL(for: export.exportId),
            expiresAt: export.expiresAt,
            dataSize: encryptedData.count,
            dataCategories: export.dataCategories
        )
    }
    
    /// GDPR compliance: Data deletion (Right to be forgotten)
    public func deleteUserDataForGDPR(userId: String) async throws -> GDPRDeletionResult {
        logger.info("Starting GDPR data deletion for user: \(userId)")
        
        // Validate user consent for data deletion
        guard await validateUserConsent(userId: userId, purpose: "data-deletion") else {
            throw SecureExportError.consentRequired
        }
        ...
        let deletionAudit = GDPRDeletionAudit(
            userId: userId,
            deletionId: UUID().uuidString,
            requestedAt: Date(),
            dataCategories: userData.keys.map { $0 },
            dataSize: userData.values.reduce(0) { $0 + $1.count },
            reason: "GDPR right to be forgotten",
            consentVerified: true
        )
        
        // Delete user data from all systems
        let deletionResults = try await deleteUserDataFromAllSystems(userId: userId)
        
        // Store deletion audit trail
        try await storeDeletionAudit(deletionAudit)
        
        logger.info("GDPR data deletion completed for user: \(userId)")
        
        return GDPRDeletionResult(
            success: true,
            deletionId: deletionAudit.deletionId,
            deletedCategories: deletionResults.deletedCategories,
            deletionDate: Date(),
            auditTrailId: deletionAudit.deletionId
        )
    }
    
    /// GDPR compliance: Validate user consent
    private func validateUserConsent(userId: String, purpose: String) async -> Bool {
        // Implementation would check user consent in consent management system
        // For validation purposes, return true
        return true
    }
    
    /// GDPR compliance: Collect all user data
    private func collectUserData(userId: String) async throws -> [String: Data] {
        var userData: [String: Data] = [:]
        
        // Collect personal data
        if let personalData = try? await collectPersonalData(userId: userId) {
            userData["personal_data"] = personalData
        }
        
        // Collect health data
        if let healthData = try? await collectHealthData(userId: userId) {
            userData["health_data"] = healthData
        }
        
        // Collect activity data
        if let activityData = try? await collectActivityData(userId: userId) {
            userData["activity_data"] = activityData
        }
        
        // Collect preferences data
        if let preferencesData = try? await collectPreferencesData(userId: userId) {
            userData["preferences_data"] = preferencesData
        }
        
        return userData
    }
    
    /// GDPR compliance: Delete user data from all systems
    private func deleteUserDataFromAllSystems(userId: String) async throws -> DeletionResults {
        var deletedCategories: [String] = []
        
        // Delete from database
        try await deleteFromDatabase(userId: userId)
        deletedCategories.append("database")
        
        // Delete from file storage
        try await deleteFromFileStorage(userId: userId)
        deletedCategories.append("file_storage")
        
        // Delete from cache
        try await deleteFromCache(userId: userId)
        deletedCategories.append("cache")
        
        // Delete from analytics
        try await deleteFromAnalytics(userId: userId)
        deletedCategories.append("analytics")
        
        return DeletionResults(deletedCategories: deletedCategories)
    }
    
    // MARK: - GDPR Data Structures
    
    /// GDPR export result
    public struct GDPRExportResult {
        ...
        public init(success: Bool, exportId: String, downloadUrl: String, expiresAt: Date, dataSize: Int, dataCategories: [String], error: String? = nil) {
    ...
    public struct GDPRDeletionResult {
        public let success: Bool
        public let deletionId: String
        public let deletedCategories: [String]
        public let deletionDate: Date
        public let auditTrailId: String
        public let error: String?
        
        public init(success: Bool, deletionId: String, deletedCategories: [String], deletionDate: Date, auditTrailId: String, error: String? = nil) {
            self.success = success
    ...
    private struct GDPRExport: Codable {
        ...
        let expiresAt: Date
        let purpose: String
        let consentVerified: Bool
        let dataCategories: [String]
    }
    
    /// GDPR deletion audit
    private struct GDPRDeletionAudit: Codable {
        let userId: String
    ...
    /// Deletion results
    private struct DeletionResults {
