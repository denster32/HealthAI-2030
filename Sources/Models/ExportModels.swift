...
struct ExportEncryptionSettings: Codable {
    ...
    private let passwordHash: Data? // SECURITY: Store hash instead of plain text
    ...
    init(encryptFile: Bool = false, password: String = "", useSecureEncryption: Bool = true) {
        ...
        self.passwordHash = password.isEmpty ? nil : Self.hashPassword(password)
        self.useSecureEncryption = useSecureEncryption
    }
    
    // SECURITY: Secure password verification
    func verifyPassword(_ password: String) -> Bool {
        guard let storedHash = passwordHash else { return password.isEmpty }
        return Self.hashPassword(password) == storedHash
    }
    
    private static func hashPassword(_ password: String) -> Data {
        // SECURITY: Use SHA-256 with salt for password hashing
        let salt = "HealthAI2030_Export_Salt".data(using: .utf8)!
        let passwordData = password.data(using: .utf8)!
        return SHA256.hash(data: salt + passwordData).data
    }
