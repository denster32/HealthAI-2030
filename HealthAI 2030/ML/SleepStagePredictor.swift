...
class SleepStagePredictor {
    ...
    init() {
        ...
        if FileManager.default.fileExists(atPath: modelURL.path) {
            ...
            Logger.warning("SleepStagePredictor.mlmodel not found, using fallback prediction", log: Logger.ml)
        }
    ...
    private func loadModel() {
        do {
            ...
            // Try to load the model directly without recompiling
            // This prevents the duplicate compilation issue
            if let compiledModelURL = try? MLModel.compileModel(at: modelURL) {
                model = try SleepStagePredictorModel(contentsOf: compiledModelURL, configuration: config)
                Logger.success("Core ML model loaded with Metal acceleration", log: Logger.aiEngine)
            } else {
                // Fallback: try to load the model directly
                model = try SleepStagePredictorModel(contentsOf: modelURL, configuration: config)
                Logger.success("Core ML model loaded directly", log: Logger.aiEngine)
            }
        } catch {
    ...
    private func metalNormalizeAndFilter(data: [Float], device: MTLDevice, commandQueue: MTLCommandQueue) async throws -> [Float] {
        ...
        normalize.encode(commandBuffer: commandBuffer, inputMatrix: createNormalizationMatrix(device: device, count: count), inputVector: inputVector, resultVector: outputVector)
    ...
    private func createNormalizationMatrix(device: MTLDevice, count: Int) -> MPSMatrix {
        ...
        let buffer = device.makeBuffer(bytes: matrixData, length: count * count * MemoryLayout<Float>.size, options: .storageModeShared)!
...
class SleepStagePredictorModel {
    ...
    // This loadModel function is now handled by the main SleepStagePredictor class
    // to prevent duplicate compilation issues
