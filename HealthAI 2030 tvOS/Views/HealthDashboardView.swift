...
struct HealthTrendsChart: View {
    ...
    var body: some View {
        VStack(alignment: .leading, spacing: 20) {
            ...
            Chart {
                ...
                ForEach(hRVData, id: \.time) { dataPoint in
                    LineMark(
                        x: .value("Time", dataPoint.time),
    ...
    private var heartRateData: [HealthDataPoint] {
        // Sample data - would come from actual health data
        return [
            HealthDataPoint(time: Date().addingTimeInterval(-3600), value: 72),
            HealthDataPoint(time: Date().addingTimeInterval(-3000), value: 75),
            HealthDataPoint(time: Date().addingTimeInterval(-2400), value: 78),
            HealthDataPoint(time: Date().addingTimeInterval(-1800), value: 80),
            HealthDataPoint(time: Date().addingTimeInterval(-1200), value: 76),
            HealthDataPoint(time: Date().addingTimeInterval(-600), value: 74),
            HealthDataPoint(time: Date(), value: 72)
        ]
    }
    
    private var hRVData: [HealthDataPoint] {
        // Sample data - would come from actual health data
        return [
            HealthDataPoint(time: Date().addingTimeInterval(-3600), value: 45),
            HealthDataPoint(time: Date().addingTimeInterval(-3000), value: 48),
            HealthDataPoint(time: Date().addingTimeInterval(-2400), value: 42),
            HealthDataPoint(time: Date().addingTimeInterval(-1800), value: 50),
            HealthDataPoint(time: Date().addingTimeInterval(-1200), value: 46),
            HealthDataPoint(time: Date().addingTimeInterval(-600), value: 44),
            HealthDataPoint(time: Date(), value: 47)
        ]
    }
}
