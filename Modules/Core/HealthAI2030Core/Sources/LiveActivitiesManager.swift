...
class LiveActivitiesManager: NSObject, ObservableObject {
    ...
    func startPeriodicUpdates() {
        Task.detached { [weak self] in
            while self?.isLiveActivityActive == true {
                await self?.updateLiveActivity()
                try? await Task.sleep(nanoseconds: UInt64(self?.updateFrequency.timeInterval ?? 5.0 * 1_000_000_000))
            }
        }
    }
    
    func stopPeriodicUpdates() {
        // No-op: async loop will exit when isLiveActivityActive is false
    }
    
    // MARK: - Health Data Management
    
    private func fetchLatestHealthMetrics() async -> HealthMetricsLiveData {
        let modelContext = try? ModelContext(for: HealthMetricsLiveData.self)
        let request = FetchDescriptor<HealthMetricsLiveData>(sortBy: [SortDescriptor(\HealthMetricsLiveData.timestamp, order: .reverse)], fetchLimit: 1)
