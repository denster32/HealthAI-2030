...
struct FamilyMember: Identifiable, Codable {
    ...
    var name: String
    var role: FamilyRole
    var healthDataPermissions: [HealthDataType: SharingPermission] // Granular permissions
}
...
enum HealthDataType: String, Codable, CaseIterable, Hashable {
...
class FamilyHealthSharingManager: ObservableObject {
    ...
    func updateSharingPermission(for memberID: UUID, dataType: HealthDataType, permission: SharingPermission) {
    ...
    func getSharingPermission(for memberID: UUID, dataType: HealthDataType) -> SharingPermission {
    ...
    private func loadFamilyMembers() {
        if let savedMembers = UserDefaults.standard.data(forKey: "familyMembers") {
            if let decodedMembers = try? JSONEncoder().decode([FamilyMember].self, from: savedMembers) {
    ...
    private func loadSharedHealthData() {
        if let savedData = UserDefaults.standard.data(forKey: "sharedHealthData") {
            if let decodedData = try? JSONEncoder().decode([SharedHealthData].self, from: savedData) {
